...
void loop() {
  if(jsData.valY < 1000) {
    moveMotors(MEC_STRAIGHT_FORWARD);
  }
  else if(jsData.valY > 3000) {
    moveMotors(MEC_STRAIGHT_BACKWARD);
  }
  else if(jsData.valX > 3000) {
    moveMotors(MEC_SIDEWAYS_RIGHT);
  }
  else if(jsData.valX < 1000) {
    moveMotors(MEC_SIDEWAYS_LEFT);
  }
  else{
    stopMotors();
  }
